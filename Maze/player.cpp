...
void Player::update() {
	...
	if (m_toggleBackTracking) {
		...
		if (sqrDist(getPosition(), getTargetPoint()) > 1) {
			toggleBackTrack(true);
			setState(State::BACKTRACK);
		}
		else {
			toggleBackTrack(false);
		...
		//need to pop so that the next location is on the next stack
		m_look.pop();
		setPosition(p);
		m_btStack.push(p);
		...
		//checks to see if the next location requires backtracking
		if (sqrDist(getPosition(), getTargetPoint()) > 1) {
			toggleBackTrack(true);
			setState(State::BACKTRACK);
		}
